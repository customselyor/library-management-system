        ...
        <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                          ...
                                          <div class="form-group">
                                            ...
                                            @php
                                                $title=null;
                                                if( count($microCategory->translations)>0 && $microCategory->translations[$k]->locale==$item->code){
                                                    $title= $microCategory->translations[$k]->title;
                                                }
                                            @endphp
                                            <input
                                              type="text"
                                              ...
                                              value="{{($title)?$title:old('title_'.$item->code)}}"
            ...
            <div class="col-4 col-sm-4 col-lg-4">
                <div class="card card-primary card-outline">
                    <div class="card-body">
                        <div class="form-group">
                            <label for="micro_parent_category_id">{{ __('messages.micro_parent_categories') }}: </label>
                             
                            <select class="form-control" id="micro_parent_category_id" name="micro_parent_category_id">
                                <option value="">-- {{ __('messages.select') }} --</option>
                                @foreach($microParentCategory as $microParentCat)
                                    @if(isset($microCategory) && $microCategory->micro_parent_category_id==$microParentCat->id)
                                        <option value="{{ $microParentCat->id }}" selected>{{ $microParentCat->translate('uz')->title }}</option>
                                    @else
                        ...
                        @php
                            $status=0;
                        @endphp
                        @if ($microCategory->count()>0)
                            @php
                                $status=$microCategory->status;
                            @endphp
                        @endif
                        <div class="form-group">
