...
class CreateMicroorganismsTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('microorganisms', function (Blueprint $table) {
            ...
            $table->foreign('micro_child_sub_category_id')
        ...
        Schema::create('microorganism_translations', function (Blueprint $table) {
            // mandatory fields
            $table->bigIncrements('id'); // Laravel 5.8+ use bigIncrements() instead of increments()
            $table->string('locale')->index();
     
            // Foreign key to the main model
            $table->unsignedBigInteger('microorganism_id');
            $table->foreign('microorganism_id','fk_microorganism')
                ->references('id')->on('microorganisms')
                ->onDelete('cascade');
